...
def V(x,t):
    ...
    b = (2+0.1*np.cos(omega*t))*(x**2)*0.5
    return b
...
def Eigen():
    ...
    a           = np.linspace(min(V(x,t)),max(V(x,t)),2000)
...
for t in t_val:
    ...
    Eigenvalue = Eigen()[0]
    Eigenvalue_stock.append(Eigenvalue)
...
'''
    ...
    normalized    : Function used to normalized the wavefunction
'''
...
for i in range(len(t_val)):
    premiere_moyenne.append( simps( np.conj(PSI_t[t_val[i]])* premiere_nouvelle_var[i]* PSI_t[t_val[i]]/(normalisation_liste[i].real**2) , x , dx=h ) )
...
for i in range(len(t_val)):
    deuxieme_moyenne.append(  simps( np.conj(PSI_t[t_val[i]])* list_derivee_seconde[i]/(normalisation_liste[i].real**2) , x , dx=h )  )
...
for i in range(len(t_val)):
    quatriem_moyenne.append( simps( np.conj(PSI_t[t_val[i]])*x*derivee_x_psi[i] /(normalisation_liste[i].real**2) , x , dx=h ) )
...
energy_moyenne = []
for i in range(len(t_val)):
    energy_moyenne.append( simps( np.conj(PSI_t[t_val[i]])* list_derivee_seconde_only[i]/(normalisation_liste[i].real**2)+ np.conj(PSI_t[t_val[i]] )* V(x,t_val[i])* PSI_t[t_val[i]]/(normalisation_liste[i].real**2) , x , dx=h ) )

autretest_valeur = []
for i in range(len(t_val)):
    autretest_valeur.append(energy_moyenne[i]/( np.sqrt( (2+0.1*np.cos(omega*t_val[i])) ) )  )
...
for i in range(1,int(len(t_val))-1):
    adiabatic_coefficient.append( ( np.sqrt(2+0.1*np.cos(omega*t_val[i-1]))-np.sqrt(2+0.1*np.cos(omega*t_val[i+1])) )/((2+0.1*np.cos(omega*t_val[i]))*2*k) )
adiabatic_coefficient.append(0)
print('Coefficient adiabatique=%.5f' %max(adiabatic_coefficient) )
...
axs[1,1].plot(x, (2+0.1*np.cos(omega*t_val[0]))*(x**2)*0.5 , color='black' , label = 'potential'      )
